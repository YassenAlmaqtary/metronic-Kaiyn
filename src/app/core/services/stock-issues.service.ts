...
export class StockIssuesService {
  ...
  private readonly basePath = '/api/StockIssue';
  private readonly typesPath = '/api/StockIssueTypes';

  getAll(): Observable<StockIssueListItem[]> {
    return this.http
      .get<ApiResponse<StockIssueListItem[]>>(buildApiUrl(this.basePath))
      .pipe(map((r) => unwrapApiResponse(r)));
  }

  getPending(): Observable<StockIssueListItem[]> {
    return this.http
      .get<ApiResponse<StockIssueListItem[]>>(buildApiUrl(`${this.basePath}/pending`))
      .pipe(map((r) => unwrapApiResponse(r)));
  }

  getById(id: number): Observable<StockIssueHeader> {
    return this.http
      .get<ApiResponse<StockIssueHeader>>(buildApiUrl(toApiPath(`${this.basePath}/{id}`, { id })))
      .pipe(map((r) => unwrapApiResponse(r)));
  }
  ...
  save(request: SaveStockIssueRequest): Observable<StockIssueHeader> {
    return this.http
      .post<ApiResponse<StockIssueHeader>>(buildApiUrl(this.basePath), request)
      .pipe(map((r) => unwrapApiResponse(r)));
  }
  ...
  getTypes(): Observable<StockIssueType[]> {
    return this.http
      .get<ApiResponse<StockIssueType[]>>(buildApiUrl(this.typesPath))
      .pipe(map((r) => unwrapApiResponse(r)));
  }

  getExchangeRate(currencyId: number): Observable<{ currencyId?: number; currencyName?: string | null; exchangeRate: number }> {
    return this.http
