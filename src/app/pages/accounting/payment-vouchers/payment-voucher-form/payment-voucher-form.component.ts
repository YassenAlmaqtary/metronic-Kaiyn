...
import {
  CreatePaymentVoucherLineRequest,
...
type PaymentLineGroup = FormGroup<{
  debitAccountId: FormControl<number | null>;
...
@Component({
  ...
  styleUrl: './payment-voucher-form.component.scss',
})
export class PaymentVoucherFormComponent implements OnInit {
  ...
  form = new FormGroup({
    ...
    beneficiaryName: new FormControl('', { nonNullable: true, validators: [Validators.required] }),
    paymentTypeId: new FormControl<number | null>(null, { validators: [Validators.required] }),
    creditAccountId: new FormControl<number | null>(null, { validators: [Validators.required] }),
    ...
    lines: new FormArray<PaymentLineGroup>([]),
  });
  ...
  get lines(): FormArray<PaymentLineGroup> {
  ...
  loadLookups(): void {
    ...
    this.paymentTypesService.getAll().subscribe({
      next: (items) => this.paymentTypes.set(items),
  ...
  loadNextNumber(): void {
    this.paymentVouchersService.getNextNumber().subscribe({
  ...
  loadVoucher(id: number): void {
    ...
    this.paymentVouchersService.getById(id).subscribe({
      next: (voucher) => {
      ...
      error: (error) => {
        this.loading.set(false);
        this.errorMessage.set(
          extractApiErrorMessage(error, this.language.translate('paymentVouchers.notFound')),
        );
      },
    });
  }

  createLine(line?: Partial<PaymentVoucherLine>): PaymentLineGroup {
    const group = new FormGroup({
      debitAccountId: new FormControl<number | null>(line?.debitAccountId ?? null, {
        validators: [Validators.required],
      }),
      amount: new FormControl(line?.amount ?? 0, {
        nonNullable: true,
  ...
  removeLine(index: number): void {
    if (this.lines.length <= 1) {
      this.errorMessage.set(this.language.translate('paymentVouchers.validation.minLines'));
      return;
    }
  ...
  onPaymentTypeChange(typeId: number | null): void {
    if (typeId == null || this.isReadOnly()) {
      return;
    }
    const paymentType = this.paymentTypes().find((item) => item.paymentTypeId === typeId);
    if (paymentType?.defaultAccountId != null) {
      this.form.controls.creditAccountId.setValue(paymentType.defaultAccountId, { emitEvent: false });
    }
  }
  ...
  onSubmit(): void {
    ...
    if (this.form.invalid) {
      this.errorMessage.set(this.language.translate('paymentVouchers.validation.form'));
      return;
    }
    ...
    const linePayload: CreatePaymentVoucherLineRequest[] = value.lines
      .filter((line) => line.debitAccountId != null)
      .map((line) => ({
    ...
    if (this.isEditMode() && this.voucherId() != null) {
      const updatePayload: UpdatePaymentVoucherRequest = payloadBase;
      this.paymentVouchersService.update(this.voucherId()!, updatePayload).subscribe({
        next: () => this.navigateSuccess('paymentVouchers.updateSuccess'),
        error: (error) => this.failSave(error),
      });
      return;
    }

    const createPayload: CreatePaymentVoucherRequest = {
      ...payloadBase,
      voucherNumber: value.voucherNumber,
    };

    this.paymentVouchersService.create(createPayload).subscribe({
      next: () => this.navigateSuccess('paymentVouchers.createSuccess'),
      error: (error) => this.failSave(error),
    });
  }
  ...
  private navigateSuccess(
    messageKey: 'paymentVouchers.createSuccess' | 'paymentVouchers.updateSuccess',
  ): void {
    this.saving.set(false);
    void this.router.navigate(['/demo1/accounting/payment-vouchers'], {
  ...
  private failSave(error: unknown): void {
    this.saving.set(false);
    this.errorMessage.set(
      extractApiErrorMessage(error, this.language.translate('paymentVouchers.saveError')),
    );
  }
}
