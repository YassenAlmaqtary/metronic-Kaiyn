...
@Component({
  ...
  styleUrl: './stock-receiving-form.component.scss',
})
export class StockReceivingFormComponent implements OnInit {
  ...
  form = new FormGroup({
    receivingNumber: new FormControl({ value: '', disabled: true }, { nonNullable: true }),
    receivingDate: new FormControl(this.today(), {
      nonNullable: true,
    ...
    receivingTypeId: new FormControl<number | null>(null),
    ...
    this.service.getById(id).subscribe({
      next: (d) => {
        ...
        this.form.patchValue({
          ...
          branchId: d.branchId,
          storeId: d.storeId,
          receivingTypeId: d.receivingTypeId ?? null,
        ...
        if (this.isReadOnly()) {
          ...
          this.form.controls.receivingNumber.disable({ emitEvent: false });
        }
      ...
      error: (e) => {
        ...
        this.errorMessage.set(
          extractApiErrorMessage(e, this.language.translate('stockReceivings.notFound')),
        );
      },
    });
  }
  ...
  onBranch(id: number | null): void {
    ...
    if (!this.isEditMode()) {
      this.service.getNextNumber(id).subscribe({
        next: (x) => this.form.controls.receivingNumber.setValue(x.voucherNumber ?? ''),
        error: () => this.form.controls.receivingNumber.setValue(''),
      });
    }
  }
  ...
  save(): void {
    ...
    if (this.form.controls.branchId.invalid) {
      missing.push(this.language.translate('stockReceivings.required.branch'));
    }
    if (this.form.controls.storeId.invalid) {
      missing.push(this.language.translate('stockReceivings.required.store'));
    }
    if (!this.form.getRawValue().receivingNumber.trim()) {
      missing.push(this.language.translate('stockReceivings.required.number'));
    }
    if (!this.details.length || this.details.invalid) {
      missing.push(this.language.translate('stockReceivings.required.details'));
    }

    if (missing.length || this.form.invalid) {
      this.errorMessage.set(
        missing.join(' — ') || this.language.translate('stockReceivings.formInvalid'),
      );
      return;
    }
    ...
    if (raw.branchId == null || raw.storeId == null) {
      this.errorMessage.set(this.language.translate('stockReceivings.formInvalid'));
      return;
    }

    this.saving.set(true);

    const payload: SaveStockReceivingRequest = {
      receivingId: this.receivingId() ?? undefined,
      receivingNumber: raw.receivingNumber.trim(),
      receivingDate: new Date(raw.receivingDate).toISOString(),
      branchId: raw.branchId,
      storeId: raw.storeId,
      receivingTypeId: raw.receivingTypeId,
      supplierId: raw.supplierId && raw.supplierId > 0 ? raw.supplierId : null,
    ...
    this.service.save(payload).subscribe({
      next: (x) => {
        ...
        const id = x.receivingId;
        void this.router.navigate(
          id
            ? ['/demo1/inventory/stock-receivings', id]
            : ['/demo1/inventory/stock-receivings'],
          { state: { successMessage: this.language.translate('stockReceivings.saveSuccess') } },
        );
      },
      error: (e) => {
        this.saving.set(false);
        this.errorMessage.set(
          extractApiErrorMessage(e, this.language.translate('stockReceivings.saveError')),
        );
      },
    });
  }
